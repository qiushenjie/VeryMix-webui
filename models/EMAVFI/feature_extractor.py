...
def pad_if_needed(x, size, window_size):
    ...
    if pad_h > 0 or pad_w > 0:  # center-pad the feature on H and W axes
        ...
        )  # (nW, Ws*Ws, 1), nW=nWindows=(h//8*scale//Ws[0])*(h//8*scale//Ws[1])
...
class InterFrameAttention(nn.Module):
    ...
    def forward(self, x1, x2, cor, H, W, mask=None):
        B, N, C = x1.shape
        B, N, C_c = cor.shape
        q = self.q(x1).reshape(B, N, self.num_heads, C //
                               self.num_heads).permute(0, 2, 1, 3)
        kv = self.kv(x2).reshape(B, -1, 2, self.num_heads, C //
                                 self.num_heads).permute(2, 0, 3, 1, 4)
        cor_embed_ = self.cor_embed(cor)
        cor_embed = cor_embed_.reshape(
            B, N, self.num_heads, self.motion_dim // self.num_heads).permute(0, 2, 1, 3)
        k, v = kv[0], kv[1]
...
class MotionFormerBlock(nn.Module):
    def __init__(self,
        ...
        self.attn = InterFrameAttention(dim, motion_dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, cor, H, W, B):
        ...
        if self.shift_size[0] or self.shift_size[1]:
            ...
            x_pad = torch.roll(
                x_pad, shifts=(-self.shift_size[0], -self.shift_size[1]), dims=(1, 2))
            cor_pad = torch.roll(
                cor_pad, shifts=(-self.shift_size[0], -self.shift_size[1]), dims=(1, 2))
            ...
            if hasattr(self, 'HW') and self.HW.item() == H_p * W_p:
                ...
                mask_windows = window_partition(
                    shift_mask, self.window_size).squeeze(-1)
                shift_mask = mask_windows.unsqueeze(
        ...
        x_win = window_partition(x_pad, self.window_size)  # (nW, Ws*Ws, proj_dim), nW=nWindows=(2*batch*h//8*scale//Ws[0])*(h//8*scale//Ws[1])
        ...
        x_appearence, x_motion = self.attn(
            x_norm, x_reverse, cor_win, H, W, shift_mask)
        ...
        if self.shift_size[0] or self.shift_size[1]:
            x_back_win = torch.roll(x_back_win, shifts=(
                self.shift_size[0], self.shift_size[1]), dims=(1, 2))
            x_motion = torch.roll(x_motion, shifts=(
                self.shift_size[0], self.shift_size[1]), dims=(1, 2))

        x = depad_if_needed(x_back_win, x.size(),
                            self.window_size).view(2*B, H * W, -1)
        x_motion = depad_if_needed(
            x_motion, cor.size(), self.window_size).view(2*B, H * W, -1)
...
class CrossScalePatchEmbed(nn.Module):
    def __init__(self, in_dims=[16, 32, 64], embed_dim=768):
        ...
        for i in range(len(in_dims)):
            for j in range(2 ** i):
                layers.append(
                    nn.Conv2d(in_dims[-1-i], base_dim, 3, 2**(i+1), 1+j, 1+j))
    ...
    def forward(self, xs):
        ...
        for i in range(len(xs)):
            for _ in range(2 ** i):
                ys.append(self.layers[k](xs[-1-i]))  # self.layers：将所有xs[]卷积成(batch*2, base_dim, h/8*scale, w/8*scale)
                k += 1
        x = self.proj(torch.cat(ys, 1))  # 只改变通道
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)  # (batch*2, h/8*scale*w/8*scale, proj_dims)
        x = self.norm(x)
...
class MotionFormer(nn.Module):
    ...
    def get_cor(self, shape, device):  # get grid
    ...
    def forward(self, x1, x2):
        ...
        for i in range(self.num_stages):
            ...
            if i < self.conv_stages:
                if i > 0:
                    x = patch_embed(x)  # 二倍下采样，通道变大
                x = block(x)  # i=0: 尺寸不变，通道变大; i>0: 尺寸不变，通道不变
                xs.append(x)
            else:
                if i == self.conv_stages:
                    x, H, W = patch_embed(xs)  # (batch*2, h/8*scale*w/8*scale, proj_dims)
                else:
                    x, H, W = patch_embed(x)
                cor = self.get_cor((x.shape[0], H, W), x.device)  # (batch*2, h/8*scale*w/8*scale, 2)
                for blk in block:
                    x, x_motion = blk(x, cor, H, W, B)
                    motion_features[i].append(x_motion.reshape(
                        2*B, H, W, -1).permute(0, 3, 1, 2).contiguous())
                x = norm(x)
                x = x.reshape(2*B, H, W, -1).permute(0, 3, 1, 2).contiguous()
                motion_features[i] = torch.cat(motion_features[i], 1)
            appearence_features.append(x)  # [0:(batch*2, embed_dims[0], h*scale, w*scale), 1: (batch*2, embed_dims[1], h/2*scale, w/2*scale), 2: (batch*2, embed_dims[2], h/4*scale, w/4*scale)]

...
from models.EMAVFI.warplayer import warp
...
class Head(nn.Module):
    ...
    def forward(self, motion_feature, x, flow):  # /16 /8 /4
        ...
        if self.scale != 4:
            x = F.interpolate(x, scale_factor=4. / self.scale,
                              mode="bilinear", align_corners=False)
        if flow != None:
            if self.scale != 4:
                flow = F.interpolate(flow, scale_factor=4. / self.scale,
                                     mode="bilinear", align_corners=False) * 4. / self.scale
            x = torch.cat((x, flow), 1)
        x = self.conv(torch.cat([motion_feature, x], 1))
        if self.scale != 4:
            x = F.interpolate(x, scale_factor=self.scale // 4,
                              mode="bilinear", align_corners=False)
...
class MultiScaleFlow(nn.Module):
    ...
    def calculate_flow(self, imgs, timestep, af=None, mf=None):
        ...
        for i in range(self.flow_num_stage):
            t = torch.full(mf[-1-i][:B].shape, timestep,
    ...
    def forward(self, x, timestep=0.5):
        ...
        for i in range(self.flow_num_stage):
            ...
            warped_img1 = warp(img1, flow[:, 2:4])
            merged.append(
                warped_img0 * mask_list[i] + warped_img1 * (1 - mask_list[i]))
